export type Module = {
  ...
  TravelTo: (Sea: number?) -> nil,
  newCachedEnemy: (Name: string, Enemy: Instance) -> nil,
  
  EquipTool: (Name: string, byTip: boolean?) -> nil,
  IsAlive: (Char: Model?) -> boolean,
  FireRemote: (...any) -> any,
  Rejoin: () -> nil,
  UseSkills: (Target: BasePart, Skills: table?) -> nil,
  IsBoss: (Name: string) -> boolean,
  
  GetEnemy: (self: Module, Name: string) -> Instance?,
  GetClosestEnemy: (self: Module, Name: string) -> Instance?,
  BringEnemies: (self: Module, ToEnemy: Instance) -> nil,
  ServerHop: (self: Module, Region: string?, MaxPlayers: number?) -> nil,
  GetEnemyByList: (self: Module, list: table) -> Instance?
  
  GetRaidIsland: (self: Module) -> Instance?
  
  GetMaterial: (self: Module, index: string?) -> number
  ItemMastery: (self: Module, index: string?) -> number
}

export type FastAttack = {
  NextAttack: number,
  Distance: number,
  ...
  attack: (self: FastAttack) -> nil,
  getBladeHits: (self: FastAttack) -> table<Instance>,
  stun: (self: FastAttack) -> boolean,
  oldBladeHits: (self: FastAttack) -> nil,
  BladeHits: (self: FastAttack) -> nil,
  attackNearest: (self: FastAttack) -> nil,
}
...
local Module = {} do
  ...
  local function GetBaseParts(Char: Model?): table
  ...
  function Module:GetAliveEnemy(Name: string, Closest: boolean?): Instance?
    local CachedEnemy = CachedEnemies[Name]
    if self.IsAlive(CachedEnemy) then
      return CachedEnemy
    end
